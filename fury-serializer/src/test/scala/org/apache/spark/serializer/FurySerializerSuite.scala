...
class FurySerializerSuite extends SparkFunSuite {
  test("FurySerializer instances are serializable") {
    val serializer = new FurySerializer(new SparkConf())
    val instance = serializer.newInstance()
    val obj = instance.deserialize[FurySerializer](instance.serialize(serializer))
    // enforce class cast
    obj.getClass
  }
